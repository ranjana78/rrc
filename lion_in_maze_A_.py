...
def manhattan_heuristic_func(n,r,c):
    return (n - 1 - r) + (n - 1 - c)
...
def search_algo(n, maze, start, end):
    ...
    C1=[0 for x in range(end+1)]
    tot_cost=[0 for x in range(end+1)]
...
#         Enter your code here
        ...
        c = pos%n
        maze[r][c]=2
        if check_pos(r+1,c,n,maze) and flag==0:
            keep1.append((r+1)*n+c)
            #print((r+1)*n+c)
            C1[(r+1)*n+c]=C1[(r)*n+c]+3
            maze[r+1][c] = -1
            tot_cost[(r+1)*n+(c)]=C1[(r+1)*n+(c)]+manhattan_heuristic_func(n,(r+1),(c))
            if ((r+1)*n+c==end):
                flag=1
        
        
        if check_pos(r,c+1,n,maze) and flag==0:
            keep1.append(r*n+(c+1))
            #print(keep1)
            C1[r*n+(c+1)]=C1[(r)*n+c]+2
            maze[r][c+1] = -1
            tot_cost[r*n+(c+1)]=C1[r*n+(c+1)]+manhattan_heuristic_func(n,r,(c+1))
            if (r*n+(c+1)==end):
                flag=1
        ...
        if pos==end or flag==1:
            print("Lion Found The meat and the Cost is", C1[end])
            print("The Cost of all the explored nodes", sum(C1))
            msg = "Lion Found The meat"
            popup_win(msg, "Congrats found", link2, screen)
            print(path)
            return
    ...
    if pos!=end:
        ...
        popup_win(msg, "Better luck next time", link1, screen)
