...
#         Enter your code here
        ...
        if  r+1 in range(n) and c in range(n) and maze[r+1][c] != 1 and maze[r+1][c] != -1 and maze[r+1][c] != 2 and flag==0:
            pos_1.append((r+1)*n+c)

            t_cost[(r+1)*n+c]=t_cost[(r)*n+c]+5
            maze[r+1][c] = -1
            if ((r+1)*n+c==end):
                flag=1
        if r in range(n) and c+1 in range(n) and maze[r][c+1] != 1 and maze[r][c+1] != -1 and maze[r][c+1] != 2 and flag==0:
            pos_1.append(r*n+(c+1))

            t_cost[r*n+(c+1)]=t_cost[(r)*n+c]+5
            maze[r][c+1] = -1
            if (r*n+(c+1)==end):
                flag=1
        ...
        if pos==end or flag==1:
            ...
            popup_win(msg, "Congratulations found", './final.png', screen)
            print("total t_cost",t_cost[end])
    ...
    if pos!=end:
        display_maze(n, maze, pos)
        redraw_maze(grid, rect, screen, n, maze, pos, delay, wid, end)
        msg = "Lion stuck in the maze."
        popup_win(msg, "Try one more time", './fail.png', screen)
